...
export default function useRarityLibrary(account, library): LibraryInterface {
  const lib = useRarityLibContract(account, library);
          ...
          resolve(
            summoners.map((value, i) => {
              return {
                ...
                gold: {
                  balance: parseInt(
                    utils.formatUnits(value.gold.balance, "ether")
                  ),
                  claimable: parseInt(
                    utils.formatUnits(value.gold.claimable, "ether")
                  ),
                  claimed: parseInt(
                    utils.formatUnits(value.gold.claimed, "ether")
                  ),
                },
                materials: {
